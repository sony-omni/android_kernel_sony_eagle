...
irqreturn_t wcd9xxx_dce_handler(int irq, void *data)
{
	...
	if (btn >= 0) {
		...
		#if 0  // BAM_S 20140910 B2828 D05914623
		wcd9xxx_update_rel_threshold(mbhc, v_btn_high[btn], vddio);
		#else
		wcd9xxx_update_rel_threshold(mbhc, v_btn_high[btn_det->num_btn - 1], vddio);
		printk("v_btn_high[btn]:%d(btn:%d), v_btn_high[btn_det->num_btn - 1]:%d(btn_det->num_btn:%d)\n", v_btn_high[btn], btn, v_btn_high[btn_det->num_btn - 1], btn_det->num_btn);
		#endif  // BAM_E 20140910
